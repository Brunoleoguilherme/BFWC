...
const T = {
  pt: {
    badge: 'Recuperar senha · Atletas',
    title: 'BFWC',
    subRequest: 'Informe o e-mail cadastrado pelo seu clube para receber o link de redefinição',
    subReset: 'Crie uma nova senha para a sua conta de atleta',
    labelEmail: 'E-mail do atleta',
    placeholderEmail: 'email@atleta.com',
  ...
  en: {
    badge: 'Reset password · Athletes',
    title: 'BFWC',
    subRequest: 'Enter the email registered by your club to receive the reset link',
    subReset: 'Create a new password for your athlete account',
    labelEmail: 'Athlete email',
    placeholderEmail: 'email@athlete.com',
  ...
  es: {
    badge: 'Recuperar contraseña · Atletas',
    title: 'BFWC',
    subRequest: 'Ingresa el correo registrado por tu club para recibir el enlace',
    subReset: 'Crea una nueva contraseña para tu cuenta de atleta',
    labelEmail: 'Correo del atleta',
    placeholderEmail: 'email@atleta.com',
...
function RecuperarSenhaAtletaInner() {
  ...
  async function handleRequest(e) {
    ...
    try {
      const res = await fetch('/api/portal/atletas/forgot-password', {
        method: 'POST',
  ...
  async function handleReset(e) {
    ...
    setLoading(true); setError(''); setInfo('');
    try {
      const res = await fetch('/api/portal/atletas/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="login-root">
      <div className="login-card" style={{ overflow: 'hidden', paddingTop: 0 }}>
        ...
        {done ? (
          <a href="/portal/atletas/login" className="login-btn" style={{ display: 'block', textAlign: 'center', background: ACCENT, color: '#fff', textDecoration: 'none', boxShadow: `0 4px 18px ${ACCENT}40` }}>
            {t.goLogin}
          </a>
        ) : isReset ? (
        ...
        <div style={{ marginTop: 20, textAlign: 'center' }}>
          <a href="/portal/atletas/login" style={{ fontSize: 13, color: '#94a3b8', textDecoration: 'none', fontWeight: 600 }}>{t.backLogin}</a>
        </div>
      </div>
    </div>
  );
...
export default function RecuperarSenhaAtletaPage() {
  return (
    <Suspense fallback={null}>
      <RecuperarSenhaAtletaInner />
    </Suspense>
  );
}
