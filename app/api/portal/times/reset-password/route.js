...
// Mesmo formato de hash usado no cadastro (register/route.js)
...
export async function POST(req) {
  try {
    ...
    if (!team) return NextResponse.json({ ok: false, message: m.invalid }, { status: 404 });

    if (!team.password_reset_expires_at || new Date(team.password_reset_expires_at) < new Date()) {
