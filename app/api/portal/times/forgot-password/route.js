...
const COPY = {
  pt: {
    ...
    badge: 'BFWC 2026 · Portal dos Times',
    title: 'Redefinir senha',
    subtitle: 'Recebemos um pedido para redefinir a senha da conta do seu clube.',
  ...
  en: {
    ...
    badge: 'BFWC 2026 · Team Portal',
    title: 'Reset password',
    subtitle: 'We received a request to reset your club account password.',
  ...
  es: {
    ...
    badge: 'BFWC 2026 · Portal de Equipos',
    title: 'Restablecer contraseña',
    subtitle: 'Recibimos una solicitud para restablecer la contraseña de la cuenta de tu club.',
...
export async function POST(req) {
  try {
    ...
    const { data: team } = await supabase
      .from('portal_teams')
      .select('id, club_name, contact_name, email')
      .ilike('email', email.trim())
      .single();

    // Resposta sempre genérica para não revelar se o e-mail existe
    if (!team) return NextResponse.json({ ok: true, message: c.generic });
    ...
    const resetUrl = `${siteUrl}/portal/times/recuperar-senha?token=${token}`;
      ...
      <td style="padding:0 28px 8px">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#081733;border:1px solid #16294d;border-radius:18px">
          <tr><td style="padding:28px 30px">
            <p style="margin:0 0 12px;font-size:15px;color:#e3e9f2;font-family:Arial,sans-serif;line-height:1.6">${c.hello(team.contact_name || team.club_name)}</p>
            <p style="margin:0 0 20px;font-size:14px;color:#9fb0c8;font-family:Arial,sans-serif;line-height:1.6">${c.body}</p>
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center">
              <tr><td align="center" style="border-radius:12px;background:#0D4BFF">
