...
export async function POST(request){
  try{
    ...
    await Promise.allSettled(adminEmails.map(to => resend.emails.send({ from:fromEmail, to, subject:`Contato BFWC: ${data.subject}`, html:`${emailLogoImg(80, 'margin:0 0 10px')}<h2>Novo contato</h2><p><b>Nome:</b> ${data.name}</p><p><b>Email:</b> ${data.email}</p><p>${data.message}</p>` })));
