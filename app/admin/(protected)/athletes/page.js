  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", color: '#0f172a' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          { label: 'Times', teams: teams.length, athletes: filteredAthletes.length, color: '#0f172a' },
          ...['Masculino', 'Feminino', 'Sub-15', 'Sub-12'].map((cat, i) => ({
            label: cat,
            teams: teams.filter(t => t.categories.includes(cat)).length,
            athletes: filteredAthletes.filter(a => a.category?.includes(cat)).length,
            color: ['#0D4BFF', '#e84dff', '#009c3b', '#d97706'][i],
          })),
        ].map(s => (
          <div key={s.label} style={{
            padding: '18px 22px', minWidth: 160, flex: '1 1 160px',
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 18 }}>
              <div>
                ...
                <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: '#94a3b8', marginTop: 4 }}>times</div>
              </div>
              <div>
                <div style={{ fontSize: 36, fontWeight: 900, letterSpacing: -2, lineHeight: 1, color: loading ? '#e2e8f0' : '#0f172a' }}>
                  {loading ? '—' : s.athletes}
                </div>
                <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: '#94a3b8', marginTop: 4 }}>atletas</div>
              </div>
            </div>
          </div>
        ))}
