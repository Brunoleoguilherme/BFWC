...
/** Aviso aos admins: parcela confirmada. */
export function adminPaymentNotifHtml({ club_name, number, plan_size, amount_cents, method }) {
  const valor = ((amount_cents || 0) / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
  const parcela = number ? `parcela ${number}${plan_size ? `/${plan_size}` : ''}` : 'pagamento';
  return `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;color:#0a1628">
      ${emailLogoImg(90, 'margin:0 0 14px')}
      <h2 style="color:#0a7d28">💰 Pagamento recebido</h2>
      <table style="border-collapse:collapse;width:100%">
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Clube</td><td style="padding:8px;border-bottom:1px solid #eee">${club_name}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Parcela</td><td style="padding:8px;border-bottom:1px solid #eee">${parcela}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Valor</td><td style="padding:8px;border-bottom:1px solid #eee">${valor}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Método</td><td style="padding:8px;border-bottom:1px solid #eee">${method}</td></tr>
        <tr><td style="padding:8px;font-weight:700">Quando</td><td style="padding:8px">${new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</td></tr>
      </table>
      <p style="margin-top:16px"><a href="https://www.brasilflagworldchampionship.com/admin/financeiro">Ver no Dashboard Financeiro</a></p>
    </div>`;
}

/** Envia o aviso de pagamento aos admins (best-effort, nunca lança). */
export async function notifyAdminsPayment(data) {
  try {
    await getResend().emails.send({
      from: fromEmail,
      to: adminEmails,
      subject: `💰 ${data.club_name} pagou ${data.number ? `a parcela ${data.number}` : ''} — BFWC 2026`.replace('  ', ' '),
      html: adminPaymentNotifHtml(data),
    });
  } catch (e) { console.error('admin payment notif error', e.message); }
}
...
// ════════════════════════════════════════════════════════════════
//  Aviso aos admins: pagamento FALHOU (cartão recusado / Pix expirado)
// ════════════════════════════════════════════════════════════════

// Traduz os motivos de recusa mais comuns do Stripe para PT-BR.
const DECLINE_REASONS_PT = {
  insufficient_funds: 'Fundos insuficientes',
  card_declined: 'Cartão recusado pelo banco',
  do_not_honor: 'Recusado pelo banco (do not honor)',
  generic_decline: 'Recusado pelo banco',
  ...
  card_not_supported: 'Cartão não suporta esta compra',
  currency_not_supported: 'Cartão não suporta a moeda (BRL)',
  card_velocity_exceeded: 'Limite de tentativas/valor excedido',
  fraudulent: 'Bloqueado por suspeita de fraude',
  merchant_blacklist: 'Bloqueado por regra antifraude',
  authentication_required: 'Autenticação (3D Secure) não concluída',
  approve_with_id: 'Banco pediu confirmação de identidade',
  try_again_later: 'Falha temporária — tentar novamente mais tarde',
};

/** Motivo amigável (PT-BR) a partir do erro do Stripe. */
export function translateDeclineReason({ decline_code, code, message } = {}) {
  if (decline_code && DECLINE_REASONS_PT[decline_code]) return DECLINE_REASONS_PT[decline_code];
  if (code && DECLINE_REASONS_PT[code]) return DECLINE_REASONS_PT[code];
  return message || decline_code || code || 'Motivo não informado';
}

/** HTML do aviso de FALHA de pagamento (para os admins). */
export function adminPaymentFailedHtml({ club_name, amount_cents, reason, method, email, extra }) {
  const valor = (amount_cents || amount_cents === 0)
    ? ((amount_cents || 0) / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
    : '—';
  const rawReason = reason || 'Motivo não informado';
  return `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;color:#0a1628">
      ${emailLogoImg(90, 'margin:0 0 14px')}
      <h2 style="color:#c0392b">⚠️ Pagamento NÃO concluído</h2>
      <p style="color:#555;margin:0 0 12px">Uma tentativa de pagamento falhou. <strong>Nenhum valor foi cobrado</strong> do time.</p>
      <table style="border-collapse:collapse;width:100%">
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Clube</td><td style="padding:8px;border-bottom:1px solid #eee">${club_name || '—'}</td></tr>
        ${email ? `<tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">E-mail</td><td style="padding:8px;border-bottom:1px solid #eee">${email}</td></tr>` : ''}
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Valor tentado</td><td style="padding:8px;border-bottom:1px solid #eee">${valor}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Método</td><td style="padding:8px;border-bottom:1px solid #eee">${method || '—'}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700;color:#c0392b">Motivo</td><td style="padding:8px;border-bottom:1px solid #eee;color:#c0392b"><strong>${rawReason}</strong></td></tr>
        ${extra ? `<tr><td style="padding:8px;border-bottom:1px solid #eee;font-weight:700">Detalhe</td><td style="padding:8px;border-bottom:1px solid #eee">${extra}</td></tr>` : ''}
        <tr><td style="padding:8px;font-weight:700">Quando</td><td style="padding:8px">${new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</td></tr>
      </table>
      <p style="margin-top:16px;color:#555">💡 Vale falar com o time para orientar (tentar outro cartão ou pagar via Pix).</p>
      <p style="margin-top:8px"><a href="https://www.brasilflagworldchampionship.com/admin/financeiro">Ver no Dashboard Financeiro</a></p>
    </div>`;
}

/** Envia o aviso de FALHA aos admins (best-effort, nunca lança). */
export async function notifyAdminsPaymentFailed(data) {
  try {
    await getResend().emails.send({
      from: fromEmail,
      to: adminEmails,
      subject: `⚠️ Pagamento falhou — ${data.club_name || 'time'} — BFWC 2026`,
      html: adminPaymentFailedHtml(data),
    });
  } catch (e) { console.error('admin payment FAILED notif error', e.message); }
}
