...
/**
 * Vagas de atleta HABILITADAS pelo que o time efetivamente garantiu (pago/isento).
 * Soma por categoria paga: Opção 1 => 20 atletas; Opção 2 => qtd contratada da categoria.
 * Sem seleção (modelo antigo/isento): Opção 2 => athletes_paid_qty; Opção 1 => 20 × categorias garantidas.
 */
export function enabledAthletesFor(team) {
  const cats = paidCategoriesOf(team);
  let sel = team?.payment_selection;
  if (typeof sel === 'string') { try { sel = JSON.parse(sel); } catch { sel = null; } }
  if (sel && typeof sel === 'object' && !Array.isArray(sel)) {
    return cats.reduce((sum, c) => sum + (String(sel[c]?.option) === '2' ? (parseInt(sel[c]?.qty, 10) || 0) : 20), 0);
  }
  if (String(team?.payment_option) === '2') return team?.athletes_paid_qty || 0;
  return 20 * cats.length;
}
